...
$("#btupload1").click(function() {
	$.ajaxFileUpload( {
		url : "/appimagerecommend/uploaImage",
		secureuri : false,
		...
		var data = eval(message);
			if (data.msg == "success") {
				...
				$("#spUpload1").html("上传成功");
				$("#imageUrl1").val(data.recommendImgUrl);
	...
	$.ajaxFileUpload( {
		...
		var data = eval(message);
			...
			if (data.msg == "success") {
				...
				$("#spUpload2").html("上传成功");
				$("#imageUrl2").val(data.recommendImgUrl);
			}else if(data.msg == "null"){
				alert("没有选择文件！");
	...
	$.ajaxFileUpload( {
		...
		var data = eval(message);
			...
			if (data.msg == "success") {
				...
				$("#spUpload3").html("上传成功");
				$("#imageUrl3").val(data.recommendImgUrl);
			}else if(data.msg == "null"){
				alert("没有选择文件！");
	...
	$.ajaxFileUpload( {
		...
		var data = eval(message);
			...
			if (data.msg == "success") {
				...
				$("#spUpload4").html("上传成功");
				$("#imageUrl4").val(data.recommendImgUrl);
			}else if(data.msg == "null"){
				alert("没有选择文件！");
...
$("#save").click(function() {
	...
	$.ajax( {
		url : "/appimagerecommend/addImage",
		type : "post",
		dataType : "json",
		data : $("#f1").serialize(),
		success : function(data) {
			if (data.msg == "success") {
				//增加成功
				alert("轮播图添加成功！");
				window.location.href = "/appimagerecommend/edit";
			} else {
				alert("系统异常，添加失败！");
			}
		},
...
$("#addIndexImage").click(function() {
	...
	var recommendImgUrl = $("#imageUrl1").val();
	if(recommendImgUrl.length==0){
		alert("须先上传图片！");
		return;
	}
	...
	$.ajax( {
		...
		success : function(data) {
			if (data.msg == "success") {
				alert("添加成功！");
				window.location.href = "/appimagerecommend/addImg?time="+ new Date();
				}
		},
		errot : function() {
			alert("系统超时，添加失败！");
...
function removePbss(obj){
	...
	if(confirm("确定要删除属性吗？")){
		$.ajax( {
			url : "/appimagerecommend/deletIndexImage",
			type : "post",
			dataType : "json",
			data : "id="+id,
			success : function(data) {
...
function modifyNum(obj){
	...
	if(confirm("确定要修改序号吗？")){
		$.ajax( {
			url : "/appimagerecommend/modifyIndexImage",
			type : "post",
			dataType : "json",
			...
			success : function(data) {
				if (data.msg == "success") {
					...
					//$(obj).parent().parent().remove();
					alert("修改成功！");
					window.location.href = "/appimagerecommend/addImg";
