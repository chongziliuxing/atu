	...
	$.ajax( {
		...
		success : function(data) {
			if (data.msg == "success") {
				//增加成功
				alert("轮播图添加成功！");
				window.location.href = "/appimagesell/edit";
			} else {
				alert("系统异常，添加失败！");
			}
		},
...
$("#addIndexImage").click(function() {
	...
	var sellImgUrl = $("#imageUrl1").val();
	if(sellImgUrl.length==0){
		alert("须先上传图片！");
		return;
	}
	...
	$.ajax( {
		...
		success : function(data) {
			if (data.msg == "success") {
				alert("添加成功！");
				window.location.href = "/appimagesell/addImg?time="+ new Date();
			}
		},
		errot : function() {
			alert("系统超时，添加失败！");
...
function removePbss(obj){
	...
	if(confirm("确定要删除属性吗？")){
		$.ajax( {
			url : "/appimagesell/deletIndexImage",
			type : "post",
			dataType : "json",
			data : "id="+id,
			success : function(data) {
...
function modifyNum(obj){
	...
	if(confirm("确定要修改序号吗？")){
		$.ajax( {
			url : "/appimagesell/modifyIndexImage",
			type : "post",
			dataType : "json",
			...
			success : function(data) {
				if (data.msg == "success") {
					...
					//$(obj).parent().parent().remove();
					alert("修改成功！");
					window.location.href = "/appimagesell/addImg";
